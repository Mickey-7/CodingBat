...
public class Array_1 {
    ...
    public int[] makeLast(int[] nums){
        ...
        int[] newArray = new int[2*totalLength];
    ...
    public int maxTriple(int[] nums){
        ...
        return first > mid && first > last ? first :
                mid > first && mid > last ? mid :
                last > first && last > mid ? last : 0;
    ...
    public boolean unlucky1(int[] nums){
        for (int i = 0; i < nums.length-1; i++) {
            if ((nums[i]==1 && nums[i+1]==3) && ((i==0 || i==1) || (i==nums.length-1 || i==nums.length-2))){
                return true;
            }
        }
        return false;
